...
def create_bash_generator_chain():
    # Initialize Ollama with Qwen-Coder2.5 for generating bash commands
    llm = Ollama(model="qwen2.5-coder:1.5b", temperature=0.2)

    # Define the prompt template for bash command generation
    prompt_template = """
    You are a tool for generating bash commands based on UIAutomator2 XML hierarchy dumps. 
    Convert the given XML data into precise ADB shell commands to interact with the UI.
    You can only generate adb shell commands with adb touch, and adb input

    Input: {xml}
    Output Bash commands:
    
    """
    prompt = PromptTemplate(template=prompt_template, input_variables=["xml"])

    # Create a chain
    return LLMChain(llm=llm, prompt=prompt)


def create_adb_filter_chain():
    # Initialize Ollama with Llama 3.2 for filtering ADB shell commands
    llm = Ollama(model="llama3.2:1b", temperature=0.0)

    # Define the prompt template for filtering valid ADB commands
    prompt_template = """
    You are a validator that ensures only valid ADB shell commands are passed. 
    Given a list of bash commands, filter out commands that are not valid ADB shell commands.

    Input Bash commands:
    {bash_commands}
    Valid ADB shell commands:
    """
    prompt = PromptTemplate(template=prompt_template, input_variables=["bash_commands"])

    # Create a chain
    return LLMChain(llm=llm, prompt=prompt)


def process_screen_dump(xml_dump):
    # Create chains
    bash_generator_chain = create_bash_generator_chain()
    adb_filter_chain = create_adb_filter_chain()

    # Generate bash commands using Qwen-Coder2.5
    bash_commands = bash_generator_chain.run({"xml": xml_dump})

    # Filter valid ADB shell commands using Llama 3.2
    valid_adb_commands = adb_filter_chain.run({"bash_commands": bash_commands})

    return valid_adb_commands
...
def dfs(nodes, package_name, device: u2.Device):
    ...
    #     {
    #         'role' : 'user',
    #         'content': 'based on whats on the screen, write one or more adb shell inputs to interact with it. Only interact with clickable elements. Use the click coordinates to interact. prefer clicking input fields or buttons over labels. Only output shell commands(input, tap or sleep)'
    #     })
    #
    ...
    f = process_screen_dump(" ".join(llm_messages))
...
messages = [{
    'role': 'system',
    'content': 'You are a tool to generate bash commands. You can generate commands using adb shell only. Output the commands in plain text, not markdown. Do not write anything extra. Give concise answers only. Whatever you write, would be used in the terminal directly.'
}]
